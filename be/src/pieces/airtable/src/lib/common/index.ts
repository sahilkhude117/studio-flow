...
export const airtableCommon = {
  ...
  //   console.log("Fetched fields from Airtable:", airtable.fields.map(f => ({ id: f.id, name: f.name, type: f.type })));
  ...
  //     console.log(`Checking field: ${name} (${key})`);
  //     console.log(`Value in input: ${name}`);    
      
  //     if (!AirtableEnterpriseFields.includes(field.type)) {

  //       if (field.type === 'multipleAttachments' && fields[name]) {
  //         newFields[key] = [
  //           {
  //             url: fields[name] as string,
  //           },
  //         ];
  //       } else if (
  //         ['multipleRecordLinks', 'multipleSelects'].includes(field.type)
  //       ) {
  //         if (Array.isArray(fields[name]) && (fields[name] as any[]).length > 0) {
  //           newFields[key] = fields[name];
  //         }
  //       } else {
  ...
  async createNewFields(
    ...
    airtable.fields.forEach((field) => {
      tableFieldMap.set(field.name.toLowerCase(), field);
    });
  
    for (const [inputFieldName, fieldValue] of Object.entries(fields)) {
      const tableField = tableFieldMap.get(inputFieldName.toLowerCase());
      
      if (!tableField) continue;
      
      console.log(`Processing field: ${inputFieldName} -> ${tableField.name} (${tableField.type})`);
      
      if (AirtableEnterpriseFields.includes(tableField.type)) continue;
  
      const fieldName = tableField.name;
  
      if (tableField.type === 'multipleAttachments' && fieldValue) {
        newFields[fieldName] = [
          {
            url: fieldValue as string,
          },
        ];
      } else if (['multipleRecordLinks', 'multipleSelects'].includes(tableField.type)) {
