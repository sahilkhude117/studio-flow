...
from studioflow_core.contracts_generated import StudioFlowApiEditorEnvVarsModel
from studioflow_core.repositories.project.project import ProjectRepository
from studioflow_core.settings import Settings
from studioflow_core.utils.code import function_called_args, subscript_called_args
...
class EnvVarsRepository:
    ...
    @staticmethod
    def list() -> List[StudioFlowApiEditorEnvVarsModel]:
        values = dotenv_values(EnvVarsRepository.get_env_var_path())
        return [
            StudioFlowApiEditorEnvVarsModel.from_dict(
                {
                    "name": k,
                    "value": v,
                }
