...
IGNORED_FILES = [".studioflow/resources.dat", ".studioflow/", "studioflow.json", "__pycache__"]
DEBOUNCE_DELAY = 0.5
...
class FileChangeEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event: FileSystemEvent):
        ...
        if filepath.resolve() == self.dot_env_path.resolve():
            StudioFlowLogger.info("Reloading .env and all modules")
        ...
        if filepath.resolve() in resolved_deps:
            StudioFlowLogger.info(f"Reloading modified module: {filepath}")
