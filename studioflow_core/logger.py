...
from studioflow_core.environment import LOGFORMAT, LOGLEVEL, NOISY_LOGLEVEL
from studioflow_core.utils.env import is_dev_env, is_test_env

internal_logger = lambda: logging.getLogger("studioflow_core")
...
class DevSDK:
    ...
    def capture_exception(cls, exception: BaseException):
        internal_logger().exception(
            msg=f"[STUDIOFLOW_LOGGER] Exception captured: {exception}"
        )

    @classmethod
    def capture_message(cls, message):
        internal_logger().info(f"[STUDIOFLOW_LOGGER] Message captured: {message}")
    ...
    def init(cls, environment: Optional[Environment]):
        ...
        try:
            cls.get_sdk().init(
                ...
                enable_tracing=True,
                release=pkg_resources.get_distribution("studioflow").version,
                shutdown_timeout=0,
            ...
            internal_logger().error(
                "[STUDIOFLOW_LOGGER] Error reporting has been turned off. "
            )
