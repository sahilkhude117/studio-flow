...
from studioflow_core.utils.dot_studioflow import CONFIG_FILE_BACKUPS
...
def migrate(data: dict, path: Path, verbose=True):
    ...
    if verbose:
        print("Your studioflow.json is outdated, running migrations...")

    try:
        filename = f"studioflow_{datetime.now().strftime('%Y%m%d%H%M%S')}.json.backup"
    ...
    if verbose:
        print("Your studioflow.json is up to date ✅")
