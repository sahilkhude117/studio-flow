...
from studioflow_core.environment import QUEUE_CONCURRENCY, RABBITMQ_EXECUTION_QUEUE
from studioflow_core.logger import StudioFlowLogger
from studioflow_core.repositories.producer import QueueMessage
from studioflow_core.utils import deserialize
...
class RabbitConsumer(Consumer):
    ...
    def _callback_threadsafe(self, msg: QueueMessage):
        if self.channel.is_open:
            StudioFlowLogger.debug(f"[CONSUMER] ack {msg.delivery_tag}")
            self.channel.basic_ack(msg.delivery_tag)
        else:
            StudioFlowLogger.capture_exception(
    ...
    def iter(self) -> Generator[QueueMessage, None, None]:
        StudioFlowLogger.warning("[CONSUMER] starting")
        while True:
            if self.stop_evt.is_set():
                StudioFlowLogger.warning("[CONSUMER] exiting iteration")
                break
    ...
    def stop(self):
        StudioFlowLogger.warning("[CONSUMER] stoping")
        self.stop_evt.set()

    def close(self):
        StudioFlowLogger.warning("[CONSUMER] channel canceling")
        self.channel.cancel()
        StudioFlowLogger.warning("[CONSUMER] channel closing")
        self.channel.close()
        StudioFlowLogger.warning("[CONSUMER] connection closing")
